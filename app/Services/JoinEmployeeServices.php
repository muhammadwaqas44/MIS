...
//        dd($request->all());
        ...
        if ($employee->probation_due_on == $request->probation_due_on) {
            $probation_due_on_status = null;
        } else {
            $probation_due_on_status = 61;
            $empHis = EmpHistory::where([['job_id', '=', $request->job_id], ['is_active', '=', 1]])->first();
            if ($empHis) {
                if ($empHis->is_active == 1) {
                    $empHis->is_active = 0;
                ...
                if ($empHis->is_active == 0) {
                    EmpHistory::create([
                        'job_id' => $job_id,
                        'call_id' => $probation_due_on_status,
                        'is_active' => 1,
                        'remarks' => $employee->probation_due_on,
                        'user_id' => auth()->user()->id,
                        'created_at' => Carbon::now()->timezone(session('timezone')),
                    ]);
                }
        ...
        if ($employee->remarks == $request->remarks) {
            $remarks = null;
        } else {
            $remarks = 62;
            $empHis = EmpHistory::where([['job_id', '=', $request->job_id], ['is_active', '=', 1]])->first();
        ...
        if ($employee->review_id == $request->review_id) {
            $review_id_app = null;
        } else {
            $review_id_app = 60;
            $empHis = EmpHistory::where([['job_id', '=', $request->job_id], ['is_active', '=', 1]])->first();
            if ($empHis) {
                ...
                if (isset($employee->employeeReview->name)) {
                    $employee_review = $employee->employeeReview->name;
                } else {
                    $employee_review = 'Has No Permanent City';
                }
                if ($empHis->is_active == 0) {
                    EmpHistory::create([
                        'job_id' => $job_id,
                        'call_id' => $review_id_app,
                        'is_active' => 1,
                        'remarks' => $employee_review,
                        'user_id' => auth()->user()->id,
                        'created_at' => Carbon::now()->timezone(session('timezone')),
                    ]);
                }
